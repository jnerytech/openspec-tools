...
import type { Component, ComponentState, Edit, PathEdit } from "../component.js";
...
function pairsAt(project: ProjectIdentity, dests: Destination[]): Pair[] {
  const pairs: Pair[] = [];
...
function destsFor(project: ProjectIdentity): Destination[] {
...
/**
 * Payload for a skill write, so applying does not have to re-derive which skill
 * goes where from the path it is about to write.
 */
 ...
 */
export const skillsComponent: Component<SkillsSelection> = {
  id: "skills",
  label: "Skills",
  ...
  inspect(project) {
    ...
    if (packaged.length === 0) return { kind: "absent" };
    ...
    if (installed.length === 0) return { kind: "absent" };
  ...
  async choose(project, ctx) {
    ...
    if (ctx.options.project) wanted.add("project");
    if (ctx.options.user) wanted.add("user");
    if (wanted.size > 0) {
  ...
  plan(project, selection) {
    if (selection === null) {
      return pairsAt(project, [only(project, "project")])
        ...
        .map(
          ({ skill, dest, path, state }): PathEdit => ({
            kind: "path",
            action: "delete",
            path,
            note:
              state.kind === "differs" ? "has local modifications" : undefined,
            payload: { skill, dest } satisfies SkillPayload,
          })
        );
    }
  ...
  applyEdit(_project, edit: Edit) {
    ...
    const { skill, dest } = edit.payload as SkillPayload;
