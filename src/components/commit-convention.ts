...
const RESTART_CAVEAT =
  "A rules directory that did not exist when your AI tool started is only " +
...
export function commitRulePath(project: ProjectIdentity): string {
  return resolve(project.root, ...FILE);
}

export const commitConventionComponent: Component<CommitConventionSelection> = {
  id: ID,
  label: "Commit convention rule",
  summary:
    "a Claude Code rule asking for one-line conventional commits (an instruction, not a check)",
  ...
  plan(project, selection) {
    ...
    const createdByUs =
      found.kind === "found" ? found.params.created === "1" : before === null;

    const content = markdownWithRegion(
      before,
      ID,
      { created: createdByUs ? "1" : "0" },
      selection === null ? null : RULE
    );
