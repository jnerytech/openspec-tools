...
import type { Component, ComponentState, Edit, RegionEdit } from "../component.js";
 ...
 */
const OFFERED: { name: string; value: string }[] = [
...
export function configPath(project: ProjectIdentity): string | null {
 ...
 */
function directive(lang: string): string[] {
  return [
...
export const artifactLanguageComponent: Component<LanguageSelection> = {
  id: "lang",
  ...
  async choose(project, ctx) {
    ...
    if (chosen !== "__other__") return { lang: chosen };
  ...
  plan(project, selection) {
    ...
    const result = yamlWithRegion(
      before,
      ID,
      selection ? { lang: selection.lang } : {},
      selection ? directive(selection.lang) : null
    );
...
export function missingConfigReason(project: ProjectIdentity): string | null {
