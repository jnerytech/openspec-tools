 ...
 */
export function compareArtifacts(a: MarkdownFile, b: MarkdownFile): number {
 ...
 */
export function parseArchivedDirName(dirName: string): ArchivedIdentity {
  const match = DATE_PREFIX.exec(dirName);
  if (!match) return { displayName: dirName };

  const [, date, rest] = match;
  if (!isCalendarDate(date)) return { displayName: dirName };
...
export async function scanArchivedChanges(
  changesDir: string
): Promise<Change[]> {
