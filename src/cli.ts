...
function resolveMode(target: string | undefined): TargetMode {
  if (!target) {
    // Default: openspec/changes/
    const changesDir = resolve(process.cwd(), DEFAULT_CHANGES_DIR);
    if (!existsSync(changesDir)) {
      console.warn(
        `[openspec-tools] Warning: ${DEFAULT_CHANGES_DIR}/ not found in current directory.\n` +
  ...
  if (stat.isDirectory()) {
    ...
    const isUnderChanges =
      abs.startsWith(changesBase) && abs !== changesBase;
