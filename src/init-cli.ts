...
import type { Component, ComponentState, Edit, PlanEntry } from "./component.js";
import {
  applyPlan,
...
/** Selected, cleared, or not mentioned at all — the third is not the second. */
type Intent = "select" | "deselect";
 ...
 */
function typedIntent(cmd: Command, key: string): Intent | undefined {
  if (cmd.getOptionValueSource(key) !== "cli") return undefined;
 ...
 */
function intentsFromFlags(cmd: Command): Map<string, Intent> {
  const intents = new Map<string, Intent>();

  const skills = typedIntent(cmd, "skills");
  if (skills) intents.set("skills", skills);

  const lang = typedIntent(cmd, "lang");
  if (lang) intents.set("lang", lang);

  const workflow = typedIntent(cmd, "claudeWorkflow");
  if (workflow) intents.set("claude-workflow", workflow);
  ...
  if (commitRule) intents.set("commit-convention", commitRule);
...
function reportStates(
  ...
  for (const component of COMPONENTS) {
    ...
    console.log(
      `  ${component.label.padEnd(32)} ${describeComponentState(state)}`
    );
  }
  ...
  if (missing) {
    ...
    console.log(
      "  It is not created here: it requires a schema value only you can choose."
    );
  }
  console.log("");
}
...
async function chooseInteractively(
  ...
  const present = COMPONENTS.filter(
    (component) => states.get(component.id)!.kind !== "absent"
  );

  const picked = await checkbox({
    message: "Check to provision, clear to remove",
    choices: COMPONENTS.map((component) => ({
      name: `${component.label}  — ${describeComponentState(
        states.get(component.id)!
      )}`,
      value: component,
  ...
  const intents = new Map<string, Intent>();
  for (const component of picked) intents.set(component.id, "select");
  for (const component of present) {
    if (!picked.includes(component)) intents.set(component.id, "deselect");
  }
  return intents;
}
...
`
  ...
  init.action(async (options: InitOptions, cmd: Command) => {
    ...
    const states = new Map<string, ComponentState>(
      COMPONENTS.map((component) => [component.id, component.inspect(project)])
    );

    const flagIntents = intentsFromFlags(cmd);
    if (flagIntents.size === 0) reportStates(project, states);
    ...
    const ctx = {
      ...
      requireInteractive: (missing: string, opts: string[]) =>
        requireInteractive(cmd, missing, opts),
    };

    const entries: PlanEntry[] = [];
    const differing: Component[] = [];
    ...
    for (const component of COMPONENTS) {
      ...
      if (state.kind === "differs") differing.push(component);

      const selection =
        intent === "select" ? await component.choose(project, ctx) : null;
      const edits = component.plan(project, selection);
      if (edits.length > 0) entries.push({ component, edits });
    }

    const all: Edit[] = entries.flatMap((entry) => entry.edits);
    if (all.length === 0) {
    ...
    for (const component of differing) {
      console.log(
        `${component.label} differs from what this package writes. The change below replaces it.`
      );
    }
    if (differing.length > 0) console.log("");

    for (const line of renderPlan(all)) console.log(line);
    ...
    console.log("");
    console.log("Done.");
    const skillsWritten = entries.some(
      (entry) =>
        entry.component.id === "skills" &&
