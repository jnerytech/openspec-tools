import { findRegion, renderRegion, spliceRegion, yamlCommentFormat, } from "./region.js";
/** The key this package splices into, and the only one it touches. */
const KEY = "context";
const DEFAULT_INDENT = "  ";
function describeScalarHeader(header) {
    const trimmed = header.trim();
    if (trimmed === "|")
        return null;
    if (trimmed === "")
        return "the key has no block scalar";
    if (trimmed.startsWith(">"))
        return "a folded scalar (>) is not spliceable";
    if (/^\|[-+0-9]/.test(trimmed)) {
        return `a block scalar with an indicator (${trimmed}) is not spliceable`;
    }
    return "the value is not a plain block scalar";
}
export function locateContext(lines) {
    ...
    if (problem)
        return { kind: "unsupported", reason: problem };
    ...
    while (bodyEnd > keyLine + 1 && lines[bodyEnd - 1].trim() === "")
        bodyEnd--;
...
export function findYamlRegion(content, id) {
    if (content === null)
        return { kind: "absent" };
    ...
    if (location.kind === "absent")
        return { kind: "absent" };
    return findRegion(lines.slice(location.bodyStart, location.bodyEnd), id, yamlCommentFormat);
}
function trailingBlanksTrimmed(lines) {
    const out = [...lines];
 ...
 */
export function yamlWithRegion(content, id, params, body) {
    ...
    if (location.kind === "absent") {
        ...
        const region = renderRegion(id, params, body, yamlCommentFormat, DEFAULT_INDENT);
    ...
    if (trailingBlanksTrimmed(nextScalar).length === 0) {
        const gapClosed = before.length > 0 &&
...
//# sourceMappingURL=region-yaml.js.map