...
import { findRegion, markdownFormat, renderRegion, spliceRegion, } from "./region.js";
...
export function findMarkdownRegion(content, id) {
    if (content === null)
        return { kind: "absent" };
 ...
 */
export function markdownWithRegion(content, id, params, body) {
    const lines = content === null ? [] : content.split("\n");
    const found = findRegion(lines, id, markdownFormat);
    const region = body === null ? null : renderRegion(id, params, body, markdownFormat);
    if (found.kind === "found")
        return tidy(spliceRegion(lines, found, region));
    if (region === null)
        return content ?? "";
...
//# sourceMappingURL=region-markdown.js.map